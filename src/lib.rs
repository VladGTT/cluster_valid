...
pub fn ball_hall_index<'py>(
    ...
    let clusters = y.as_array().into_shape(1).unwrap();

    // {
    //     let mut temp = clusters.lock().unwrap();
    //     temp.subscribee.add_subscriber(&clusters_centroids);
    //     temp.subscribee.add_subscriber(&index);
    // }
    //
    // clusters_centroids
    //     .lock()
